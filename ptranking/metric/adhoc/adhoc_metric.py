...
def torch_rankwise_err(batch_rankings, max_label=None, k=10, point=True, device='cpu'):
	assert batch_rankings.size(1) >= k
	assert max_label is not None # it is either query-level or corpus-level

	batch_labels = batch_rankings[:, 0:k]
	t2 = torch.tensor([2.0], dtype=torch.float, device=device)
	batch_satis_probs = (torch.pow(t2, batch_labels) - 1.0) / torch.pow(t2, max_label)
	...
	batch_ranks = torch.arange(k, dtype=torch.float, device=device).expand_as(batch_labels) + 1.0
...
def torch_nerr_at_k(batch_predict_rankings, batch_ideal_rankings, k=None, device=None, label_type=LABEL_TYPE.MultiLabel, max_label=None):
	valid_max_cutoff = batch_predict_rankings.size(1)
	cutoff = min(valid_max_cutoff, k)

	if LABEL_TYPE.MultiLabel == label_type:
		if max_label is None: max_label = torch.max(batch_ideal_rankings)
		batch_sys_err_at_k = torch_rankwise_err(batch_predict_rankings, max_label=max_label, k=cutoff, point=True, device=device)
		batch_ideal_err_at_k = torch_rankwise_err(batch_ideal_rankings, max_label=max_label, k=cutoff, point=True, device=device)
...
def torch_ndcg_at_k(batch_predict_rankings, batch_ideal_rankings, k=None, device='cpu', label_type=LABEL_TYPE.MultiLabel):
	batch_sys_dcg_at_k = torch_dcg_at_k(batch_predict_rankings, cutoff=k, label_type=label_type, device=device) # only using the cumulative gain at the final rank position
	batch_ideal_dcg_at_k = torch_dcg_at_k(batch_ideal_rankings, cutoff=k, label_type=label_type, device=device)
	batch_ndcg_at_k = batch_sys_dcg_at_k / batch_ideal_dcg_at_k
	return batch_ndcg_at_k

def torch_ndcg_at_ks(batch_predict_rankings, batch_ideal_rankings, ks=None, device='cpu', label_type=LABEL_TYPE.MultiLabel):
	valid_max_cutoff = batch_predict_rankings.size(1)
