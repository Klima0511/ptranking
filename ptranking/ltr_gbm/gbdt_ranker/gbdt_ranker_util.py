...
def loss_function(batch_preds, batch_std_labels,device):
        ...
        batch_delta_ndcg = get_delta_ndcg(batch_ideal_rankings=batch_stds_sorted,
                                          ...
                                          label_type=LABEL_TYPE.MultiLabel, device=device)
        '''
        len_scores = batch_predict_rankings.size(1)
        # 初始化一个零张量用于保存 lambda 值和 hessian 值
        lambda_values = torch.zeros_like(batch_predict_rankings)
        hessian_values = torch.zeros_like(batch_predict_rankings)
        
        # 遍历所有的文档对
        for i in range(len_scores):
            for j in range(i + 1, len_scores):
                delta_pred = batch_preds[0][i] - batch_preds[0][j]
                p = torch.sigmoid(delta_pred)
                g = torch.Tensor(p - 1.0).to(device)
                h = torch.Tensor(max(p * (1.0 - p), eps)).to(device)
                w = batch_delta_ndcg[0, i, j]
                lambda_values[0][i] += g * w
                lambda_values[0][j] -= g * w
                hessian_values[0][i] += 2.0 * w * h
                hessian_values[0][j] += 2.0 * w * h

        
        batch_s_ij = torch.unsqueeze(batch_descending_preds, dim=2) - torch.unsqueeze(batch_descending_preds, dim=1)
        batch_p_ij = torch.sigmoid(1.0 * batch_s_ij)
        batch_grad_ij = batch_p_ij  - 1
        #batch_hess_ij =  batch_p_ij * (1.0 -  batch_p_ij)
        #batch_hess_ij = batch_hess_ij.clamp(min=1e-16)
        
        lambda_ij = batch_delta_ndcg * batch_grad_ij
        lambda_i = lambda_ij.sum(dim = 2)
        #hess_ij =  batch_hess_ij * batch_delta_ndcg * 2
        #hess_i = hess_ij.sum(dim = 2)
        '''
...
def lambdarank_objective(yhat, y, sample_weight=None, device=None, **kwargs):
    ...
    for i in range(group.shape[0]):
        ...
        ngbm_preds =  ngbm_preds.detach()
        ngbm_preds.requires_grad = True
...
def rankmse_objective(yhat, y, sample_weight=None, device=None, **kwargs):
    ...
    for i in range(group.shape[0]):
        ...
        batch_stds = y[head:head + gr].view(1, -1)
        ngbm_preds = yhat[head:head + gr].view(1, -1)
        batch_loss = _loss_function(ngbm_preds, batch_stds,device)
        '''
        batch_preds_sorted, batch_preds_sorted_inds = torch.sort(ngbm_preds, dim=1,
                                                                 descending=True)  # sort documents according to the predicted relevance
        batch_stds_sorted, _ = torch.sort(batch_stds, dim=1, descending=True)
        ...
        batch_loss = torch.sum(torch.sum(_batch_loss, dim=(2, 1)))
        '''
        loss_function_sum = lambda ngbm_preds: _loss_function(ngbm_preds, batch_stds,device)
        batch_grad_order1 = grad(batch_loss, ngbm_preds)[0]
        hess_matrix = torch.autograd.functional.hessian(loss_function_sum, ngbm_preds, vectorize=True)
        #hess_matrix = hessian(loss_function)(ngbm_preds)
        # hess = hvp(loss_function, (ngbm_preds,), (jnp.ones_like(ngbm_preds),))
