...
import torch
from torch.distributions import Normal, NegativeBinomial, Poisson, StudentT, LogNormal, Laplace, Uniform,\
    TransformedDistribution, SigmoidTransform, AffineTransform, Gamma, Gumbel, Weibull

from ptranking.ltr_gbm.base.gbm.gbm import GBM

##- PGBM-specific torch.jit.script -##
@torch.jit.script
...
def _predict_forest_muvar(X: torch.Tensor, nodes_idx: torch.Tensor, nodes_split_bin: torch.Tensor,
    ...
    for estimator in range(1, best_iteration):
        variance_total += lr ** 2 * variance[estimator] - 2 * lr * corr * variance_total.sqrt() * variance[
            estimator].sqrt()
...
class PGBM(GBM):
    ...
    """
    def __init__(self, id='PGBM', gbm_para_dict=None, gpu=False, device=None, distributed=False):
        #super(PGBM, self).__init__(id=id, size=size, rank=rank, gbm_para_dict=gbm_para_dict, gpu=gpu, device=device, distributed=distributed)
        super(PGBM, self).__init__(id=id, gbm_para_dict=gbm_para_dict, gpu=gpu, device=device, distributed=distributed)

    def init_params(self, gbm_para_dict):
        pass
    ...
    def train(self, train_set, objective, metric, params=None, valid_set=None, obj_id=None,
              sample_weight=None, eval_sample_weight=None, ranking_obj=False, ranking_metric=False):
        self.train_op(train_set, objective, metric, params=params, valid_set=valid_set, create_tree=_create_tree,
                      sample_weight=sample_weight, eval_sample_weight=eval_sample_weight,
                      ranking_obj=ranking_obj, ranking_metric=ranking_metric, probabilistic=True, obj_id=obj_id)
    ...
    def predict_dist(self, X, n_forecasts=100, parallel=True, output_sample_statistics=False):
        ...
        if self.distribution == 'normal':
            ...
            yhat = TransformedDistribution(base_dist,
                                           [SigmoidTransform().inv, AffineTransform(loc, scale)]).rsample(
                [n_forecasts])
            ...
            def weibull_params_grad(k):
                ...
                hess = -(2 * gamma(2 / k) * (
                            2 * psi0(2 / k) ** 2 - 2 * k * psi0(1 / k) - psi1(1 / k) + 2 * k * psi0(
                        2 / k) - 4 * psi0(1 / k) * psi0(2 / k) + 2 * psi1(2 / k) + 2 * psi0(1 / k) ** 2)) / (
                                   k ** 4 * gamma(1 / k) ** 2)
    ...
    def permutation_importance(self, X, y=None, n_permutations=10, levels=None):
        ...
        for feature in range(n_features):
            ...
            if y is not None:
                ...
                permutation_importance_metric[feature] = (yhat_base.unsqueeze(0) - yhat).abs().sum(
                    1) / yhat_base.sum() * 100
    ...
    def optimize_distribution(self, X, y, distributions=None, tree_correlations=None):
        ...
        if self.verbose > 1:
            print(
                f'Lowest CRPS: {crps_best:.4f} (Distribution: {distribution_best}, Tree correlation: {tree_correlation_best:.3f})')
