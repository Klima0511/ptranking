...
from ptranking.ltr_gbm.base.pgbm.pgbm import PGBM
...
from ptranking.ltr_gbm.gbdt_ranker.gbdt_ranker_util import nDCG_metric

class PGBMRanker(PGBM):
    """ A group of learning-to-rank models based on GBDT """
    def __init__(self, id='PGBMRanker', gbm_para_dict=None, gpu=False, device=None, distributed=False):
    ...
    def full_train(self, train_data, vali_data):
        self.pgbm_ranker = PGBM(gbm_para_dict=self.gbm_para_dict, gpu=self.gpu, device=self.device, distributed=False)
...
class PGBMRankerParameter(ModelParameter):
    ''' Parameter class for PGBMRanker '''
    def __init__(self, debug=False, para_json=None):
        super(PGBMRankerParameter, self).__init__(model_id='PGBMRanker', para_json=para_json)
        self.debug = debug

    def default_para_dict(self):
        self.gbm_para_dict = dict(model_id=self.model_id,
                                  obj='mse', learning_rate =0.01, max_bin =256, max_leaves =32,
    ...
    def grid_search(self):
        ...
        if self.use_json:
            ...
            choice_obj = ['mse'] if self.debug else ['mse']
